...
public class CharacterMovement : MonoBehaviour
{
    ...
    void Dodge()
    {
        // 움직일 때 jump키 누르면 구르기, 점프나 회피 둘중 하나만 행동하기 위해서
        if (isGrounded && !isDodge)
        {
            // 구르는 방향 움직이는 방향으로,
            DodgeVec = moveVec.magnitude > 0 ? moveVec.normalized : transform.forward;
            isDodge = true;
            // 구르기 속도 반영
            rb.velocity = DodgeVec * moveSpeed * 10;
            // 시간차 두어 회피폼에서 돌아오기
